...
Route::middleware(['admin'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::get('/dashboard/pdf', [AdminDashboardController::class, 'generatePdf'])->name('dashboard.pdf');
    Route::get('/georeferenciacion', [GeoController::class, 'index'])->name('geo');
    ...
    Route::get('/users/{id}/billing', [\App\Http\Controllers\Admin\AdminUsersController::class, 'billing'])->name('users.billing');
    Route::post('/users/{id}/assign-plan', [\App\Http\Controllers\Admin\AdminUsersController::class, 'assignPlan'])->name('users.assignPlan');
    Route::post('/facturas/{userId}/store', [\App\Http\Controllers\Admin\AdminUsersController::class, 'storeFactura'])->name('facturas.store');
    Route::delete('/users/{id}', [\App\Http\Controllers\Admin\AdminUsersController::class, 'destroy'])->name('users.destroy');

    // Perfil del administrador
    Route::get('/profile', [\App\Http\Controllers\Admin\AdminProfileController::class, 'index'])->name('profile');
    Route::post('/profile/name', [\App\Http\Controllers\Admin\AdminProfileController::class, 'updateName'])->name('profile.updateName');
    Route::post('/profile/email', [\App\Http\Controllers\Admin\AdminProfileController::class, 'updateEmail'])->name('profile.updateEmail');
    Route::post('/profile/password', [\App\Http\Controllers\Admin\AdminProfileController::class, 'updatePassword'])->name('profile.updatePassword');

    // Gestión de apiarios
    Route::get('/apiarios', [\App\Http\Controllers\Admin\AdminApiariosController::class, 'index'])->name('apiarios.index');
    Route::get('/apiarios/{id}', [\App\Http\Controllers\Admin\AdminApiariosController::class, 'show'])->name('apiarios.show');
    Route::delete('/apiarios/{id}', [\App\Http\Controllers\Admin\AdminApiariosController::class, 'destroy'])->name('apiarios.destroy');

    // Rutas para apiarios eliminados (papelera)
    Route::get('/apiarios-eliminados', [\App\Http\Controllers\Admin\AdminApiariosController::class, 'deleted'])->name('apiarios.deleted');
    Route::post('/apiarios-eliminados/{id}/restore', [\App\Http\Controllers\Admin\AdminApiariosController::class, 'restore'])->name('apiarios.restore');
    Route::delete('/apiarios-eliminados/{id}/permanent', [\App\Http\Controllers\Admin\AdminApiariosController::class, 'permanentDelete'])->name('apiarios.permanent-delete');
});
...
Route::middleware(['auth', 'check.payment'])->group(function () {
    ...
    // Rutas para registro de Producción/Cosecha (DESHABILITADAS - Se usa estimación automática desde índice de cosecha)
    // Route::get('visitas/create5/{apiario}', [VisitaController::class, 'createProduccion'])->name('visitas.create5');
    // Route::post('visitas/create5/{apiario}', [VisitaController::class, 'storeProduccion'])->name('visitas.store5');
    // Route::get('/generate-document/produccion-record/{apiarioId}', [DocumentController::class, 'generateProduccionDocument'])->name('generate.document.produccion');
    // Route::get('visitas/create5/{apiario}/{visita}/edit', [VisitaController::class, 'editProduccion'])->name('visitas.produccion.edit');

...
<body>
  ...
  <table>
    <tr>
      <th>Nº Transacción / Folio</th>
      <td>{{ $payment->receipt_number ?? $payment->buy_order }}</td>
    </tr>
    <tr>
      <th>Fecha y hora</th>
      <td>{{ optional($payment->receipt_issued_at)->format('d/m/Y H:i') }}</td>
    </tr>
    <tr>
      <th>Monto total</th>
    ...
    <tr>
      <th>Medio de pago</th>
      <td>{{ $payment->receipt_payment_method ?? 'Tarjeta' }}</td>
    </tr>
    <tr>
      <th>Emisor</th>
      <td>{{ $empresa['razon'] }} — RUT {{ $empresa['rut'] }}</td>
    </tr>
    <tr>
      <th>Cliente</th>
      <td>{{ $user->email }}</td>

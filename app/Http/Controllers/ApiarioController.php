...
class ApiarioController extends Controller
{
    ...
    public function obtenerConsejo($apiario_id)
    {
        ...
        return response()->json([
            ...
            'consejo' => $consejo,
            'apiario' => $apiario->id,
            'puede_registrar_pcc' => false, // No permitir registrar mientras tanto
            
        ]);

        /*
        // Buscar último registro completo en visitas de tipo "Sistema Experto"
        $ultimaVisita = $apiario->visitas()
            ->where('tipo_visita', 'Sistema Experto')
            ->latest()
            ->first();
        // FKs de las 7 tablas con todos los PCC asociados
        $requisitosPCC = [
            'desarrollo_cria_id', 'calidad_reina_id', 'estado_nutricional_id',
            'presencia_varroa_id', 'presencia_nosemosis_id', 'indice_cosecha_id', 'preparacion_invernada_id'
        ];
        // Detectar si falta algun PCC
        $faltantes = collect($requisitosPCC)->filter(fn($c) => !$ultimaVisita || !$ultimaVisita->$c);
        if ($faltantes->count() > 0) {
            // Si falta algún PCC, retornar un mensaje de error y link a registrar PCC
            return response()->json([
                'success' => false,
                'message' => 'Debes registrar un PCC completo antes de generar el consejo.',
        ...
        // Simulación realista de consejo (elige uno al azar)
        $consejos = [
            "Monitorea varroa y fortalece la alimentación proteica antes del invierno.",
            "El apiario muestra buen desarrollo, revisa reservas y la postura de la reina.",
            "Sugiero rotar cuadros y aplicar tratamiento preventivo para Nosema.",
